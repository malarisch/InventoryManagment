...
## To-Do
- Überarbeite die Implementierung des Metadaten-Konzzepts. Derzeit ist alles einfach in einer Card angeordnet, also ein großer Block. Das ist recht unhandlich, außerdem fehlen viele Felder, die in den Typen beschrieben wurden. Das "Soll" wäre: Alle Metadaten Kategorien (physische eigenschaften, stromversorgung, etc.) haben ihre eigenen Karten. Diese Karten sind auf oberster Ebene zusammen mit den Basisdaten, etc. im Interface angezeigt. Außerdem werden nicht alle Karten direkt angezeigt: Es existiert eine Karte "Hinzufügen" in der Buttons sind, um die anderen Karten zu spawnen. Beim Vererben werden die Karten auch bei den Objekten, die die Daten geerbt haben, angezeigt.  Zeige einfach die geerbten Values als Platzhalter an mit einer Checkbox daneben "Ignorieren": Wenn die gesetzt wird den tatsächlichen Wert auf NULL, den Input deaktivieren. Der Fix dafür überschneidet sich wahrscheinlich mit dem Todo für die fehlenden Metadaten felder.
- Implementiere das Konzept "Kontakte" - also Datenbankseitig eine stumpfe Tabelle mit Kontakten entsprechend der Metadaten Definitionen. Jobs bekommen dann als Auftraggeber einfach einen Foregin key auf den jeweiligen Kontakt, beim Job anlegen kann man einen Kontakt auswählen oder anlegen. Gleiches gilt für die "Supplier", wie sie bei Artikeln und Equipments beschrieben sind.
