...
export function createAdminClient() {
  const url = (process.env.PAGES_DYNAMIC) ? process.env["NEXT_PUBLIC_SUPABASE_URL"]! : process.env.NEXT_PUBLIC_SUPABASE_URL!;
  const serviceRole = (process.env.PAGES_DYNAMIC) ? process.env["SUPABASE_SERVICE_ROLE_KEY"]! : process.env.SUPABASE_SERVICE_ROLE_KEY!;
  if (!url || !serviceRole) {
