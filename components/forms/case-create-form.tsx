    ...
    try {
      ...
      if (assetTagTemplateId !== "") {
        ...
        const printed_code = companyMeta
          ? buildAssetTagCode(companyMeta, "case", id, templatePrint, { company_name: companyName })
          : String(id);
    ...
    <form onSubmit={onSubmit} className="grid grid-cols-1 md:grid-cols-12 gap-6">
      <Card className="md:col-span-5">
        <CardHeader>
          ...
          <CardDescription>Case‑Titel und Equipment</CardDescription>
        </CardHeader>
        <CardContent className="space-y-3">
          ...
          <div className="grid gap-2">
            ...
            <select id="case_equipment" className="h-9 rounded-md border bg-background px-3 text-sm" value={caseEquipment} onChange={(e) => setCaseEquipment(e.target.value === "" ? "" : Number(e.target.value))}>
              <option value="">— Kein Case‑Equipment —</option>
              {equipments.map((e) => {
                const aName = articles.find((a) => a.id === e.article_id)?.name ?? (e.article_id ? `#${e.article_id}` : "—");
                return (<option key={e.id} value={e.id}>#{e.id} • {aName}</option>);
              })}
            </select>
          </div>
        </CardContent>
              ...
              <tbody>
                {equipments
                  .filter((e) => {
                    ...
                    return (String(e.id).includes(term) || aName.toLowerCase().includes(term));
                  })
      ...
      <Card className="md:col-span-7">
        <CardHeader>
          ...
          <CardDescription>Artikel mit Menge hinzufügen</CardDescription>
        </CardHeader>
        <CardContent className="space-y-2">
          <div className="flex items-center gap-2">
            ...
            <input className="h-9 w-24 rounded-md border bg-background px-3 text-sm" type="number" min={1} value={articleAmount} onChange={(e) => setArticleAmount(Math.max(1, Number(e.target.value) || 1))} />
      ...
      <Card className="md:col-span-5">
        ...
        <CardContent>
          <div className="grid gap-2">
            ...
            <select id="asset_tag_template" className="h-9 rounded-md border bg-background px-3 text-sm" value={assetTagTemplateId} onChange={(e) => setAssetTagTemplateId(e.target.value === "" ? "" : Number(e.target.value))}>
              <option value="">— Keins —</option>
              {assetTagTemplates.map((t) => (<option key={t.id} value={t.id}>{`Template #${t.id}`}</option>))}
            </select>
          </div>
        </CardContent>
      </Card>
