...
- **IMPORTANT**: After each completed task, update `agentlog.md` with a concise summary (2-5 lines: date/time, task name, key files touched, next step) AND include this update in the SAME commit as your code changes. Do NOT create separate documentation commits.
- Append entries to agentlog chronologically; no need for elaborate formatting.
...
- After each completed task, create a git commit that captures the changes you made ALONG WITH the agentlog.md update.
